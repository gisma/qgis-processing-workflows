  ...
  # 3. Reproject layers to user-defined CRS.
  # 4. Apply field calculations (ENVIMET_ID, SURFACE_TYPE).
  # 5. Extract buildings (`buildings.gpkg`).
  # 6. Extract buildings heights using DSM and DEM for height analysis.
  #
  # Dependencies:
  # - QGIS (`qgis_process`)
  # - GDAL (`ogr2ogr`)
  ...
  usage() {
      echo "Usage:"
      echo "  Positional Mode: $0 <input_osm> [<dsm_raster> <dem_raster> <target_crs> <extent>]"
      echo "  Named Arguments: $0 -i <input_osm> [-s <dsm_raster>] [-d <dem_raster>] -c <target_crs> -e <extent>"
      echo -e "\nArguments:"
      ...
      echo "  -c <target_crs>  Target CRS in EPSG format (e.g., EPSG:32632) (required)"
      echo "  -e <extent>      Clipping extent in 'xmin,ymin,xmax,ymax [EPSG:xxxx]' format (required)"
  ...
  if [[ "$1" == "-"* ]]; then
      # Named argument mode
      while getopts ":i:s:d:c:e:h" opt; do
          case ${opt} in
              i) input_osm="$OPTARG" ;;
              s) dsm_raster="$OPTARG" ;;
              d) dem_raster="$OPTARG" ;;
  ...
  if [[ -z "$input_osm" || -z "$target_crs" || -z "$extent" ]]; then
      echo "Error: The OSM file, target CRS, and extent are required."
      usage
  fi
  ...
  echo "Using target CRS: $target_crs"
  echo "Using clipping extent: $extent"
 ...
 # extent=$extent
  ...
  filter_multipolygons_surface="\"landuse\" IN ('farmland', 'residental', 'industrial' ) OR \"natural\" IN  ('water','wetland')"
  ...
  formula_multipolygons_surface_envimet="CASE
        ...
        WHEN \"natural\" = 'water' THEN '0200WW'
        WHEN \"natural\" = 'wetland' THEN '0200LI'
        ELSE '02AGSS'
  END"
  ...
  echo -e "\e[1;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\e[0m"
  ogr2ogr -f "GPKG"  "$output_osm_gpkg" "$input_osm" || {
...
for layer in "${!layers[@]}"; do
  ...
  qgis_process run native:zonalstatisticsfb \
      ...
      STATISTICS=2 \
      OUTPUT="$output_zonal_stats">>"$output_dir/qgis_${research_area}.log" || exit 1
  ...
  qgis_process run native:extractbyextent \
      INPUT="$output_zonal_stats" \
        EXTENT="$extent" \
        CLIP="true" \
      OUTPUT="$final_buildings">>"$output_dir/qgis_${research_area}.log" || exit 1
  else
  ...
  qgis_process run native:buffer \
      ...
      SEGMENTS=5 \
      DISSOLVE="false" \
      OUTPUT="$tmp_buffer">>"$output_dir/qgis_${research_area}.log"  || exit 1
  ...
  qgis_process run native:extractbyextent \
      ...
      EXTENT="$extent" \
      CLIP="true" \
      OUTPUT="$clip_surface_buffer">>"$output_dir/qgis_${research_area}.log"  || exit 1
  ...
  qgis_process run native:fieldcalculator \
      ...
      OUTPUT="$clip_surface_buffer_envimet">>"$output_dir/qgis_${research_area}.log" || exit 1
  ...
  qgis_process run native:mergevectorlayers \
     ...
     CRS="$target_crs" \
     OUTPUT="$final_surface">>"$output_dir/qgis_${research_area}.log"  || exit 1
